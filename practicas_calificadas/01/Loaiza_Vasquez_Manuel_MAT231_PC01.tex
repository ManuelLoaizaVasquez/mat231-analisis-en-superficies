...
\begin{proof}
  Tenemos
  \[
    0 \leq \bint_{R_i} f_i \leq L(f_i, \pi_i)
  \]
  para cualquier partici\'on $\pi_i$ de $R_i$.
  En consecuencia,
  \[
    0 \leq \bint_{R_1} f_1 \bint_{R_2} f_2 \leq L(f_1, \pi_1) L(f_2, \pi_2).
  \]
  Por transitividad obtenemos
  \[
    \bint_{R_1} f_1 \bint_{R_2} f_2 \leq L(F, \pi_1 \times \pi_2).
  \]
  y al ser esto una cota inferior para cualquier par de particiones, acontece
  \[
    \bint_{R_1} f_1 \bint_{R_2} f_2 \leq
    \bint_{R_1 \times R_2} F \leq
    L(F, \pi_1 \times \pi_2).
  \]
  An\'alogamente obtenemos la desiguladad por la derecha
  \[
    U(F, \pi_1 \times \pi_2) \leq
    \tint_{R_1 \times R_2} F \leq
    \tint_{R_1} f_1 \tint_{R_2} f_2.  
  \]

  Utilizamos el inciso $4.3$ para poder atar los cabos sueltos
  \[
    \bint_{R_1} f_1 \bint_{R_2} f_2 \leq
    \bint_{R_1 \times R_2} F \leq
    \tint_{R_1 \times R_2} F \leq
